...
/// bridge (the S4/S6 seam idiom) — the crate-level definition
...
/// The crate-level definition (`crate::BlockOn`), re-exported at this
/// path unchanged.
...
mod wire_tests {
    ...
    /// The entitlements response step.
    fn entitlements(status: &'static str, body: Vec<u8>) -> Step {
        Step::static_bytes("GET", ENTITLEMENTS_PATH, status, body)
    }
    ...
    fn fetch_round_trips_the_recorded_fixture() {
        let (handle, port, seen) = spawn_scripted(vec![
            Step::anon_session(),
            entitlements("200 OK", FIXTURE.as_bytes().to_vec()),
        ]);
    ...
    fn fetch_maps_envelope_refusals() {
        ...
        let (handle, port, _seen) = spawn_scripted(vec![
            Step::anon_session(),
            entitlements("200 OK", refused.into_bytes()),
        ]);
        let (adapter, _rt) = adapter_against(port);
