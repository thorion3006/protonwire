...
//! snapshot.
//!
//! Two gates protect generation (the S13 validation stays THE gate —
...
fn render(doc: &GenFile, rows: &[(String, String, String)]) -> Result<String> {
    ...
    out.push_str(
        "//! GENERATED by `cargo xtask groups-gen` — DO NOT EDIT BY HAND.\n\
         ...
         //! document that fails the S13 golden-table validation) and the vendored\n\
         //! UN M49 snapshot `resources/geo/un-m49.csv` (checksum-gated by\n\
         //! `cargo xtask m49-verify`; generation embeds only a snapshot that\n\
         //! verifies). `cargo xtask all` checks this file's freshness; the core\n\
         //! suite pins its semantics (T-28/T-29/T-30/T-33).\n\n",
    );
    out.push_str(
